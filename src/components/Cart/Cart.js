            ...
            if(context.cart.length>0)
            {
                const showButton=()=>{
                    if(this.state.User===null){
                        return(
                                <ThanhToanButton setUser={setUser}/>
                        )
                    }
                    else{
                        return(
                            <div>
                                ...
                                <PayPalButton
                                    ...
                                    onApprove={async (data, actions) => {await onApprove(data, actions); addDonHang(true);context.setDHID(this.state.DONHANG_ID);addCTDH(context.cart);context.clearDATA()}}
                                />
                            </div>
                        )
                    }
